...
export default function Header() {
  ...
  const handleSearchSubmit = (e: FormEvent) => {
    ...
    router.push(trimmedQuery ? `/events?query=${encodeURIComponent(trimmedQuery)}` : '/events');
        ...
        <ul className={`mobile-menu flex items-start min-[1025px]:items-center gap-4 min-[1025px]:gap-6 text-base xl:text-lg font-medium max-[1025px]:flex-col max-[1025px]:absolute max-[1025px]:top-full max-[1025px]:left-0 max-[1025px]:w-full bg-[#FCFFF7] dark:bg-[#222222] max-[1025px]:shadow-lg max-[1025px]:py-4 max-[1025px]:px-5 ${isMobileMenuOpen ? 'flex' : 'hidden'} min-[1025px]:flex rounded-b-2xl`}>
          ...
          {!isMounted ? (
            <>
              ...
              <li><span className="inline-flex whitespace-nowrap items-center gap-1 p-2 text-gray-400/50 animate-pulse"><FingerPrintIcon className="inline-block size-5" /><span>S&apos;Inscrire</span></span></li>
            </>
          ) : status === 'loading' ? (
            <li>Chargement...</li>
          ) : session ? (
            <>
              <li><Link href="/my-events" className={`inline-flex whitespace-nowrap items-center gap-1 transition-all ease-in-out duration-600 dark:hover:text-[#ff952aff] rounded-full p-2 hover:shadow-[inset_0px_2px_1px_gray] ${pathname === '/my-events' ? ' shadow-[inset_0px_2px_1px_#101828]  dark:shadow-[inset_0px_2px_1px_#ff952aff]' : ''}`} onClick={() => setIsMobileMenuOpen(false)}><CalendarDateRangeIcon className="inline-block size-5 -translate-y-0.5" /><span>Mes Inscriptions</span></Link></li>
            ...
            <>
              <li><Link href="/login" className={`inline-flex whitespace-nowrap items-center gap-1 transition-all ease-in-out duration-600 dark:hover:text-[#ff952aff] rounded-full p-2 hover:shadow-[inset_0px_2px_1px_gray] ${pathname === '/login' ? ' shadow-[inset_0px_2px_1px_#101828]  dark:shadow-[inset_0px_2px_1px_#ff952aff]' : ''}`} onClick={() => setIsMobileMenuOpen(false)}><FingerPrintIcon className="inline-block size-5" /><span>Se Connecter</span></Link></li>
              <li><Link href="/register" className={`inline-flex whitespace-nowrap items-center gap-1 transition-all ease-in-out duration-600 dark:hover:text-[#ff952aff] rounded-full p-2 hover:shadow-[inset_0px_2px_1px_gray] ${pathname === '/register' ? ' shadow-[inset_0px_2px_1px_#101828]  dark:shadow-[inset_0px_2px_1px_#ff952aff]' : ''}`} onClick={() => setIsMobileMenuOpen(false)}><FingerPrintIcon className="inline-block size-5" /><span>S&apos;Inscrire</span></Link></li>
            </>
          )}
        </ul>
